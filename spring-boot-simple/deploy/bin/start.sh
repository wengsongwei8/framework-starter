...
#启动类
MAIN_CLASS=com.weng.framework.SampleApiStarterApplication
...
#开启远程调试可以启用以下命令：
#nohup $_EXECJAVA $JAVA_OPTS  -Xdebug -Xrunjdwp:server=y,transport=dt_socket,address=8087,suspend=n  -classpath $CLASSPATH $MAIN_CLASS  >${LOG_PATH} 2>&1 &
tail -f ${LOG_PATH}
