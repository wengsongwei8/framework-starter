...
export default {
  ...
  methods: {
    ...
    _afterInitDataFun(response) {
      // TODO 成功初始化数据后的实现 抽像函数，继承函数需实现
    },
    initView(id, row) {
      this.formData = {}
      ...
      if (dataFunRes){
        dataFunRes.then((response) => {
          if (isRequestSuccess(response)) {
            ...
            this._afterInitDataFun(response);
          } else {
            this.formData = {}
            this.$notify({ title: '失败', message: '获取'+ this.domainName + '失败', type: 'error', duration: 2000 })
          }
