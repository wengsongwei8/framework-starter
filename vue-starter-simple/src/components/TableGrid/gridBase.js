...
export default {
  ...
  methods: {
    ...
    _afterDeleteSuccess(response) { // 处理删除成功
      // TODO 抽像函数，继承函数需实现
    },
    getOpColums(){ // 列表操作按钮
        ...
        if (config.isOnlyRowView){ // 只显示查看按钮
          config.opBody = (h, row) =>
            <div style="padding:0px;margin:0px;" align="center">
               <el-button type="text" icon="el-icon-view"   onClick={() => this.toView(row)} >查看</el-button>
            </div>
        }else if (config.isOnlyRowEdit){ // 只显示编辑按钮
          config.opBody = (h, row) =>
            <div style="padding:0px;margin:0px;" align="center">

              <el-button type="text" icon="el-icon-edit"   onClick={() => this.toEdit(row)} >编辑</el-button>
            </div>
        } else if (config.isOnlyRowDel){ // 只显示查看按钮
          config.opBody = (h, row) =>
            <div style="padding:0px;margin:0px;" align="center">
              <el-button type="text" icon="el-icon-delete"   onClick={() => this.handleDelete(row)} >删除</el-button>
            </div>
        }else {
          config.opBody = (h, row) => [
            <div style="padding:0px;margin:0px;" align="center">
    ...
    getOpBtnBody(h, row, config){ // 获取操作内容
      return <div style="width: 130px;text-align: left;">
    ...
    getOpBtnBodyOfView(h, row, config){ // 获取查看操作内容
      if (config && config.isRowView){
        return <el-button type="text" icon="el-icon-view"   onClick={() => this.toView(row)} >查看</el-button>
    ...
    getOpBtnBodyOfEdit(h, row, config){ // 获取编辑操作内容
      ...
      if (config && config.isRowEdit){
        return <el-button type="text" icon="el-icon-edit"   onClick={() => this.toEdit(row)} >编辑</el-button>
      } else {
        return ""
      }
    },
    getOpBtnBodyOfDel(h, row, config){ // 获取删除操作内容
      ...
      if (config && config.isRowDel){
        return <el-button type="text" icon="el-icon-delete"   onClick={() => this.handleDelete(row)} >删除</el-button>;
    ...
    handleDeleteResponse(response, successMessage, errorMessage) { // 删除后的操作
      ...
      if (util.isRequestSuccess(response)) {
        ...
        this._afterDeleteSuccess(response);
        this.handleSearch()
      } else {
